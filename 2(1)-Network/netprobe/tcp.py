...
def connect_with_fallback(ips: list[str], port: int, timeout: float, prefer: str = "any") -> TCPConnectResult:
    ...
    if prefer == "ipv4":
        ipv4 = [ip for ip in ips if ":" not in ip] #ips를 순회하면서 ipv4만 모아 리스트로 반환
        ipv6 = [ip for ip in ips if ":" in ip]
        ordered = ipv4 + ipv6
    elif prefer == "ipv6":
        ipv6 = [ip for ip in ips if ":" in ip] #ips를 순회하면서 ipv6만 모아 리스트로 반환
        ipv4 = [ip for ip in ips if ":" not in ip]
        ordered = ipv6 + ipv4
    else: 
        ordered = list(ips) # 둘다 아니면 원본 순서대로 리스트 복사
    ...
    for ip in ordered:
        ...
        if sock is None: # IP 실패시 다음 IP로 넘어감
            last_err = err
            continue

        local_addr = sock.getsockname() # socket의 로컬 주소를 튜플로 반환
        peer_addr = sock.getpeername() # 연결된 서버 주소

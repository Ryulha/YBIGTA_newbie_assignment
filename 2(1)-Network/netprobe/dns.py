...
def resolve(host: str) -> tuple[list[str], Optional[str]]:
    ...
    try:
        ...
        for info in infos:
            ip = info[4][0] # socket.getaddrinfo를 통해 가져온 값 - ip는 5번째 원소인 sockaddr에 포함
            if ip not in ips: # 이미 추가된 IP면 건너뛰기
                ips.append(ip)
...
def pick_ip(ips: list[str], prefer: str = "any") -> Optional[str]:
    ...
    if prefer == "ipv4":
        for ip in ips:
            if ":" not in ip: # 콜론이 없으면 IPv4로 판단
                return ip
    elif prefer == "ipv6":
        for ip in ips:
            if ":" in ip: # 콜론이 있으면 IPv6로 판단
                return ip
